            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                       let pp = './https://telegra.ph/file/938c546fac46c5aa019c0.jpg'
                        try {
                            pp = await this.profilePictureUrl(user, 'image')
                        } catch (e) {
                        } finally {
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Yah,si Anggota Baru Telah Bergabung @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                (chat.sBye || this.bye || conn.bye || 'Anggota Telah Berkurang @user!')).replace('@user', '@' + user.split('@')[0])
